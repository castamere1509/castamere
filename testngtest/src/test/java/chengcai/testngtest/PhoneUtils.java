...
public class PhoneUtils {
    ...
    public static boolean phoneTe(String phone) {

        String regex = "^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(17[013678])|(18[0,5-9]))\\d{8}$";
        if(phone.length() != 11){
            return false;//手机号码不是11位就返回false
        }else{
            Pattern p = Pattern.compile(regex);
            Matcher m = p.matcher(phone);
            boolean isMatch = m.matches();
            if(isMatch){            	
            	return true;//手机号码符合规则返回true
            } else {
            	return false;//手机号码不符合规则返回false
            }
        }
    }
	...
	public static void main(String[] args) {
		...
		boolean bo =PhoneUtils.phoneTe("15555555555");
