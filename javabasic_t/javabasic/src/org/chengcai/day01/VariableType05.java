...
public class VariableType05 {
	...
	public static void main(String[] args) {
		...
		byte bt = 127;//byte用来保存整数，保存的范围是在-128 到 127之间
		short st =32000;//short用来保存整数，保存范围正负3万左右
		int i =2100000000;//int用来保存整数,保存的值范围在正负21亿左右
		long lg =1000000000000000000L;//保存整数，在整数类型后面加L或者l,代表该数字为long类型
		
		//注意：Java中所有的整数字面默认int类型，在赋值的时候会进行自动类型转换
        //注意：在Java中当到了某个类型的最高值时再加1，那么会变成最小值
		//声明为long 类型的时候，需要在数字后面加大写的L，或者小写的l
		
		
		
		//保存小数的有两种  float , double
		
		float f1 = 3.5F;//如果数字为小数类型，莫认是double类型，所以float声明的时候必须加F或者f,
		                //代表这个小数是float
		double db =3.5;// 可以在后面加D或者d表示是个double类型，但是不需要，因为小数默认为double类型
		
		//注意double类型的精度比float进度高，所以保存小数一般用double
       ...
       //八种基本类型以外的都是引用类型，保存字符串使用String类
		String str ="aadsad";
